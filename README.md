...
|---|---|---|
| `GET` | `/healthz` | `200 {status:"ok"}` si Redis responde, `503` si no |
...
`Authorization: Bearer <accessToken>` (el token que emite `POST /api/v1/auth/login`;
se verifica aquí con el mismo `JWT_ACCESS_SECRET`, sin llamar a la API).
...
ahora), `source` `NOTIFICATION` o `SMS`. Límite de 120 por minuto por usuario
(`429`). La misma notificación del mismo usuario el mismo día (hora de Ciudad de
México) se responde como duplicada sin encolarla; la API vuelve a deduplicar con
su propia huella.
...
| `RATE_PER_MINUTE` | `120` | Por usuario |
